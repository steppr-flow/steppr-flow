package io.thalyazin.agent.registration;

import com.fasterxml.jackson.databind.ObjectMapper;
import io.thalyazin.agent.AgentProperties;
...
/**
 * Client that registers workflows with the Thalyazin monitoring server.
 */
@Component
@ConditionalOnProperty(name = "thalyazin.agent.server-url")
...
public class WorkflowRegistrationClient {
    ...
    public void shutdown() {
        try {
            ...
            log.warn("Failed to unregister from server: {}", e.getMessage());
        }
    }

    /**
     * Register all workflows with the server.
     */
    public void registerWorkflows() {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                log.info("Successfully registered {} workflows with server at {}",
            ...
            log.warn("Failed to register workflows with server {}: {}",
    ...
    public void unregister() {
        ...
        try {
            ...
            log.info("Unregistered from server");
        } catch (Exception e) {
     ...
     * If the server was down and comes back up, or if the server doesn't recognize
     * this instance (returns 404), re-register automatically.
     */
    @Scheduled(fixedRateString = "${thalyazin.agent.heartbeat-interval-seconds:30}000")
    public void heartbeat() {
        ...
        try {
            ...
            if (response.statusCode() == 404) {
                ...
                log.trace("Heartbeat sent to server");
            }
        } catch (Exception e) {
            if (serverAvailable) {
                ...
                log.debug("Failed to send heartbeat (server unavailable): {}", e.getMessage());
                // Attempt re-registration - if server is back up, this will succeed
                registerWorkflows();
            }
        }
    }
