 ...
 * <p>
 * This controller provides endpoints specific to the server module:
 ...
 * use the /api/workflows endpoints provided by thalyazin-monitor.
 */
@RestController
...
public class DashboardController {
    ...
    @Operation(summary = "Get dashboard configuration", description = "Get UI configuration settings (title, refresh interval, dark mode, base path)")
    ...
    @Operation(summary = "Get dashboard overview", description = "Get dashboard overview data including statistics, workflows, and recent executions")
    ...
    @Operation(summary = "Get workflow definitions", description = "Get all workflow definitions with their steps and configuration. Supports filtering by topic, serviceName, and status.")
     ...
     * Supports filtering by topic (partial match), serviceName (exact match), and status (exact match).
     */
    private List<Map<String, Object>> getCombinedWorkflowDefinitions(String topicFilter, String serviceNameFilter, String statusFilter) {
        ...
        if (registryService != null) {
            for (RegisteredWorkflow rw : registryService.getAllWorkflows()) {
                ...
                // Apply filters
                if (!matchesFilters(rw.getTopic(), workflowServiceName, workflowStatus, topicFilter, serviceNameFilter, statusFilter)) {
                    continue;
                }
        ...
        if (workflowRegistry != null) {
            for (var def : workflowRegistry.getAllDefinitions()) {
                ...
                // Apply filters
                if (!matchesFilters(def.getTopic(), "local", workflowStatus, topicFilter, serviceNameFilter, statusFilter)) {
                    continue;
                }
