...
public class WorkflowFailureHandler {
     ...
     */
    public void handleFailure(WorkflowMessage message, StepDefinition step,
        ...
        if (!retryInfo.isExhausted() && isRetryable(cause)) {
            ...
            if (definition.getOnFailureMethod() != null) {
                try {
                    callbackInvoker.invokeRaw(definition.getOnFailureMethod(), definition.getHandler(), message, cause);
                } catch (Exception ex) {
    ...
    private void scheduleRetry(WorkflowMessage message, RetryInfo retryInfo, String errorMessage) {
        ...
        WorkflowMessage retryMessage = messageFactory.createRetryMessage(message, newRetryInfo);
