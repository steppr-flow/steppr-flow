...
import io.thalyazin.core.annotation.OnFailure;
import io.thalyazin.core.annotation.OnSuccess;
import io.thalyazin.core.annotation.Step;
import io.thalyazin.core.annotation.Timeout;
import io.thalyazin.core.annotation.Topic;
 ...
 * Scans for @Topic annotated beans on startup.
 * Implements SmartInitializingSingleton to ensure all beans are created before scanning.
 */
...
public class WorkflowRegistry implements SmartInitializingSingleton {
    ...
    public void afterSingletonsInstantiated() {
        ...
        for (Map.Entry<String, Object> entry : beans.entrySet()) {
            ...
            if (!(bean instanceof Thalyazin workflow)) {
                log.warn("Bean {} has @Topic but doesn't implement Thalyazin", entry.getKey());
                continue;
            }
            ...
            log.info("Registered workflow: topic={}, steps={}", topic.value(), definition.getTotalSteps());
        }
    ...
    private WorkflowDefinition buildDefinition(Topic topic, Thalyazin handler, Class<?> beanClass) {
        ...
        for (Method method : beanClass.getDeclaredMethods()) {
            ...
            if (stepAnnotation != null) {
                ...
                Duration timeout = timeoutAnnotation != null
                        ? Duration.of(timeoutAnnotation.value(), timeoutAnnotation.unit().toChronoUnit())
                        : null;
        ...
        Duration timeout = workflowTimeout != null
                ? Duration.of(workflowTimeout.value(), workflowTimeout.unit().toChronoUnit())
                : null;
        ...
        return WorkflowDefinition.builder()
                ...
                .handler(handler)
                .handlerClass((Class<? extends Thalyazin>) beanClass)
                .steps(steps)
     ...
     */
    public WorkflowDefinition getDefinition(String topic) {
