package io.thalyazin.core.security;
 ...
 * <ul>
 *   <li>Wildcards (*) are not used, preventing deserialization of arbitrary classes</li>
 *   <li>Package names follow valid Java naming conventions</li>
 *   <li>At least one trusted package is configured</li>
 * </ul>
 *
 * <p>Using wildcards in trusted packages is a security vulnerability that can
 * lead to Remote Code Execution (RCE) attacks through malicious JSON payloads.
 *
...
public final class TrustedPackagesValidator {
     ...
     * @throws IllegalArgumentException if the list is null, empty, or contains invalid package names
     * @throws SecurityException if wildcards are detected
     */
    public static void validate(List<String> trustedPackages) {
        if (trustedPackages == null) {
            throw new IllegalArgumentException("Trusted packages cannot be null");
        }

        if (trustedPackages.isEmpty()) {
            throw new IllegalArgumentException("At least one trusted package must be configured");
        }
    ...
    private static void validatePackage(String pkg) {
        ...
        if ("*".equals(pkg)) {
            throw new SecurityException(
                ...
                "This is a security vulnerability that can lead to Remote Code Execution. " +
                "Please specify explicit package names like 'io.thalyazin.core.model'."
            );
        }
        ...
        if (pkg.contains("*")) {
            throw new SecurityException(
                "Wildcard patterns are not allowed in trusted packages: '" + pkg + "'. " +
        ...
        if (!VALID_PACKAGE_PATTERN.matcher(pkg).matches()) {
            throw new IllegalArgumentException(
                ...
                "Package names must follow Java naming conventions (e.g., 'com.example.domain')."
            );
        }
    }
}
